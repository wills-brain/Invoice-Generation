...
def normalize_fisher_order_candidate(raw: str) -> str:
    """Normalize the Fisher Scientific Order Number for Customer PO.

    Correct Customer PO value is the DR-prefixed value beside/under
    "Fisher Scientific Order Number". OCR often reads the leading DR as DBR,
    D8R, DBS, DRS, or fragments the suffix with spaces/punctuation. This keeps
    only a DR + numeric suffix suitable for the Customer PO Number field.
    """
    raw_text = str(raw or "").upper()
    # Keep a reviewable local token but remove table noise and labels.
    token = re.sub(r"[^A-Z0-9]", "", raw_text)
    if not token:
        return ""

    # Normalize common OCR variants of the visible DR prefix.
    # Examples seen in Thermo/Fisher scans:
    #   DBR2IB5A0 -> DR218340
    #   DBs eWl4  -> likely DR5214 after OCR corrections
    # The ``DBS/D8S/D5S`` variants often mean the OCR read the second digit of
    # the order number as part of the prefix, so preserve that S as a 5.
    if token.startswith(("DBS", "D8S", "D5S")):
        token = "DR5" + token[3:]
    else:
        prefix_variants = ("DBR", "D8R", "DER", "DPR", "DRS", "D5R")
        for prefix in prefix_variants:
            if token.startswith(prefix):
                token = "DR" + token[len(prefix):]
                break
    if not token.startswith("DR"):
        # OCR sometimes drops the R and starts DB / D8 / DS.
        if token.startswith(("DB", "D8", "DS", "D5")):
            token = "DR" + token[2:]
        elif token.startswith("D") and len(token) >= 5:
            token = "DR" + token[1:]

    if token.startswith("DR"):
        suffix_raw = token[2:]
        # Frequent OCR artifacts in the suffix.
        suffix_raw = suffix_raw.replace("5A0", "340").replace("SA0", "340")
        suffix_raw = suffix_raw.replace("EW", "2").replace("VW", "14").replace("W", "1")
        suffix = suffix_raw.translate(str.maketrans({"O": "0", "Q": "0", "I": "1", "L": "1", "B": "8", "S": "5", "Z": "2", "T": "7"}))
        # Keep the first sufficiently long numeric sequence after DR. This avoids
        # pulling customer-account or terms text into the PO field.
        m = re.search(r"(\d{3,12})", suffix)
        if m:
            return "DR" + m.group(1)
    return ""
...
def parse_thermofisher_po(text: str, filename: str) -> ParsedPO:
    text = normalize_ocr_text(text)
    purchase_order_number = find_purchase_order_number(text, filename)
    fisher_order_number = find_fisher_scientific_order_number(text, filename)
...
def render_product_header() -> None:
    st.markdown("# Simplex Sciences ThermoFisher Invoice Processing")
    st.caption("For internal use by Simplex Sciences Operations team.")

def main() -> None:
    st.set_page_config(page_title="Simplex Sciences ThermoFisher Invoice Processing", layout="wide")
    apply_brand_style()
    ...
    with st.sidebar:
        ...
        st.caption("Invoice date and sales rep apply to every uploaded PDF. Shipping date, internal order number, shipping cost, and FedEx tracking are entered per PDF below.")
        st.markdown("---")
    ...
    pdf_files = st.file_uploader("Upload Thermo Fisher / Fisher Scientific purchase order PDFs", type=["pdf"], accept_multiple_files=True)
    ...
    for idx, pdf in enumerate(pdf_files):
        ...
        parsed = parse_thermofisher_po(text, pdf.name)
        data = asdict(parsed)
        ...
        with st.expander("User Entry Required for this PDF", expanded=True):
            ...
            with u3:
                per_shipping_cost = st.text_input("Shipping cost", value="45.93", key=f"shipping-cost-{idx}")
            with u4:
        ...
        with col1:
            ...
            if edited.get("customer_po_number") and not str(edited.get("customer_po_number", "")).upper().startswith("DR"):
